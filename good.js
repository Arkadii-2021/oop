...
class GoodsList {
	constructor (goods, goodsFiltered, filter, sortPrice, sortDir) {
		this.goods = [];
		this.goodsFiltered = [];
		this.filter = filter;
	...
	filterName (filter) {
		...
		this.goods.forEach(function (good, indexIdReal) {
			let str = f1.goods[indexIdReal].name;
...
class Basket extends GoodsList {
	constructor (id, name, description, sizes, price, available, amount, basket, goods) {
		super(id, name, description, sizes, price, available, goods);
		this.basket = [];
	};
...
f1.add(1, 'ФУТБОЛКА ANTISOCIAL BASIC ФИОЛЕТОВЫЙ', 'Футболка выполнена из 100% хлопка.', [], 1650, 1);
